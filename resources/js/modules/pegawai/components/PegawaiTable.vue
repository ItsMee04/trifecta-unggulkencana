<template>
    <div
        ...
        <div class="overflow-x-auto relative">
            ...
            <div v-if="isLoading && paginatedPegawai.length > 0"
            ...
            <table class="w-full text-left border-collapse table-fixed">
                <thead>
                    <tr
                        ...
                        <th class="py-3 px-4 w-[42%]">Pegawai</th>
                        <th class="py-3 px-4 w-[20%]">Jabatan</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800 text-sm">
                    ...
                    <tr v-if="isLoading && paginatedPegawai.length === 0">
                        ...
                        <td class="py-3 px-4 font-medium text-slate-900 dark:text-slate-200 uppercase">
                            <div class="flex items-center gap-3">
                                <div class="relative w-9 h-9 flex-shrink-0">
                                    <img v-if="item.image"
                                         :src="`/storage/pegawai/image/${item.image}?t=${new Date().getTime()}`"
                                         alt="Avatar"
                                    ...
                                    <div v-else class="w-full h-full rounded-full bg-indigo-100 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-xs font-bold">
                                        {{ item.nama.charAt(0) }}
                                    </div>
                                </div>
                                <div class="flex flex-col min-w-0">
                                    <span class="truncate font-semibold text-slate-800 dark:text-slate-200">{{ item.nama }}</span>
                                    <span class="text-xs text-slate-400 normal-case tracking-wide font-normal">{{ item.nip }}</span>
                                </div>
                            </div>
                        </td>
                    ...
                    <tr v-if="!isLoading && paginatedPegawai.length === 0">
                        <td colspan="5" class="py-10 text-center text-slate-400 dark:text-slate-500 text-xs">
                            No data available.
                        </td>
                    </tr>
                </tbody>
...
import { usePegawai } from '../composables/usePegawai'; // Disesuaikan ke composable pegawai
