        ...
        <div class="flex min-h-full items-center justify-center p-4 text-center">
            ...
            <Transition appear enter-active-class="ease-out duration-300"
                ...
                <div v-show="isModalOpen"
                    ...
                    <form @submit.prevent="submitDiskon" class="p-6 space-y-4 font-sans">
                        <div class="space-y-1.5">
                            <label for="diskon" class="text-sm font-medium text-slate-900 dark:text-slate-200 mb-3 block">
                                Nama Diskon
                            </label>
                            <input v-model="formDiskon.diskon" type="text" id="diskon"
                            ...
                            <span v-if="errors.diskon" class="text-xs text-rose-500 font-medium block mt-1">
                                {{ errors.diskon }}
                            </span>
                        </div>

                        <div class="space-y-1.5">
                            <label for="nilai" class="text-sm font-medium text-slate-900 dark:text-slate-200 mb-3 block">
                                Nilai
                            </label>
                            <input v-model="formDiskon.nilai" type="text" id="nilai"
                                placeholder="Nilai diskon dalam (%)"
                                class="w-full px-3.5 py-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm focus:outline-none focus:border-indigo-500 dark:focus:border-indigo-600 text-slate-900 dark:text-white transition" />
                            <span v-if="errors.nilai" class="text-xs text-rose-500 font-medium block mt-1">
                                {{ errors.nilai }}
                            </span>
                        </div>
...
const { isModalOpen, isEdit, formDiskon, errors, isLoading, submitDiskon, closeModal } = useDiskon();
</script>
