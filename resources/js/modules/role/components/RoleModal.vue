        ...
        <div class="flex min-h-full items-center justify-center p-4 text-center">
            ...
            <Transition appear enter-active-class="ease-out duration-300"
                ...
                <div v-show="isModalOpen"
                    ...
                    <form @submit.prevent="submitRole" class="p-6 space-y-4 font-sans">
                        <div class="space-y-1.5">
                            <label for="nama_role" class="text-sm font-medium text-slate-900 dark:text-slate-200 mb-3 block">
                                Nama Role
                            </label>
                            <input v-model="formRole.role" type="text" id="nama_role"
                            ...
                            <span v-if="errors.role" class="text-xs text-rose-500 font-medium block mt-1">
                                {{ errors.role }}
                            </span>
                        </div>
...
const { isModalOpen, isEdit, formRole, errors, isLoading, submitRole, closeModal } = useRole();
</script>
