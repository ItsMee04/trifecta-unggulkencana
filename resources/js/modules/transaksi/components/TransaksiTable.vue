<template>
    <div
        ...
        <div
            ...
            <h2 class="font-bold text-slate-900 dark:text-white text-base">Daftar Transaksi Pelanggan</h2>
        ...
        <div class="w-full overflow-x-auto relative">
            ...
            <div v-if="isLoading && paginatedTransaksi.length > 0"
            ...
            <table class="w-full text-left border-collapse min-w-[600px]">
                <thead>
                    <tr
                        ...
                        <th class="py-3 px-4 w-[25%]">PRODUK</th>
                        <th class="py-3 px-4 w-[20%]">PELANGGAN</th>
                        <th class="py-3 px-4 w-[10%]">DISKON</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800 text-sm">
                    ...
                    <tr v-else v-for="(item, index) in paginatedTransaksi" :key="item.id"
                            ...
                            <div class="flex items-center gap-3">
                                <div class="flex flex-col min-w-0">
                                    <span class="truncate font-semibold text-slate-800 dark:text-slate-200">
                                        {{ item.transaksidetail?.[0]?.produk?.kodeproduk || '-' }}
                                    </span>
                                    <span class="text-xs text-slate-400 normal-case tracking-wide font-normal">
                                        {{ item.transaksidetail?.[0]?.produk?.nama || '-' }}
                                    </span>
                                </div>
                            </div>
                        </td>
                        ...
                        <td
                            class="py-3 px-4 font-medium text-slate-900 dark:text-slate-200 uppercase truncate">
                            {{ item.diskon?.nilai }} %
                        </td>
