            ...
            <table class="w-full border-collapse">
                ...
                <thead class="print-header">
                    ...
                    <tr
                        ...
                        <th class="py-2 w-[14%] text-right pr-3">HARGA</th>
                    </tr>
                </thead>
                ...
                <tbody class="divide-y divide-slate-200 text-[10px]">
                    <tr v-for="(item, index) in laporanData.items" :key="index" class="break-inside-avoid">
                        ...
                        <td class="py-2 pr-3 text-right font-mono font-medium text-slate-900">
                            {{ formatRupiah(item.harga || item.total_per_item) }}
                        </td>
                    </tr>
                </tbody>
...
const fetchLaporanData = async () => {
    ...
    try {
        ...
        const response = await laporanService.getLaporanPenjualan(payload);

        if (response && response.status) {
            laporanData.value = response.laporanData;
            document.title = `LAPORAN-PENJUALAN-${tanggalAwal}_sd_${tanggalAkhir}`;
        }
    } catch (error) {
        console.error("Gagal memuat data laporan penjualan:", error);
    } finally {
