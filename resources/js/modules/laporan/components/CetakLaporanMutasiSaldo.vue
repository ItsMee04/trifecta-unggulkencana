    ...
    <div class="min-h-screen bg-zinc-800 flex flex-col items-center justify-start p-6 print:bg-white print:p-0">
        ...
        <div v-if="isLoading" class="my-auto text-white font-semibold animate-pulse print:hidden">
            <div class="flex flex-col items-center gap-3">
                ...
                <span>Menyiapkan Laporan Mutasi Saldo...</span>
            </div>
        </div>
            ...
            <table class="w-full border-collapse">
                ...
                <thead class="print-header">
                    ...
                    <tr
                        ...
                        <th class="border-r border-slate-300 py-2 w-[10%] text-center">REKENING</th>
                        <th class="border-r border-slate-300 py-2 w-[22%] text-center">KETERANGAN</th>
                        <th class="border-r border-slate-300 py-2 w-[10%] text-center">JENIS</th>
                        <th class="border-r border-slate-300 py-2 w-[15%] text-center">DEBIT</th>
                        <th class="border-r border-slate-300 py-2 w-[15%] text-center">KREDIT</th>
                        <th class="border-r border-slate-300 py-2 w-[15%] text-center">SALDO</th>
                    </tr>
                </thead>
                                    ...
                                    <div class="w-[280px] space-y-1 text-[10px]">
                                        ...
                                        <div class="flex justify-between border-b border-slate-200 pb-1">
                                            ...
                                            <span class="font-mono font-bold">
                                                {{ laporanData.summary?.total_transaksi || 0 }}
                                            </span>
                                        </div>
                                        ...
                                        <div class="flex justify-between pt-0.5">
                                            ...
                                            <span
                                                ...
                                                {{ formatRupiah(laporanData.summary?.saldo_akhir || 0) }}
                                            </span>
                                        </div>
...
const fetchLaporanData = async () => {
    ...
    try {
        ...
        const response = await laporanService.getLaporanMutasiSaldo(payload);

        if (response && response.status) {
            laporanData.value = response.laporanData;
            document.title = `LAPORAN-MUTASI-SALDO-${tanggalAwal}_sd_${tanggalAkhir}`;
        }
    } catch (error) {
        console.error("Gagal memuat data laporan mutasi saldo:", error);
    } finally {
