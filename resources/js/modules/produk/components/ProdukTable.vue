<template>
    <div
        ...
        <div
            ...
            <h2 class="font-bold text-slate-900 dark:text-white text-base">Daftar Pegawai</h2>
            ...
            <div class="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end">
                ...
                <div class="relative flex-1 sm:flex-none sm:w-64">
                    ...
                    <input v-model="searchQuery" type="text" placeholder="Search By Nama / Kode Produk ..."
        ...
        <div class="w-full overflow-x-auto relative">
            ...
            <div v-if="isLoading && paginatedProduk.length > 0"
            ...
            <table class="w-full text-left border-collapse min-w-[600px]">
                <thead>
                    <tr
                        ...
                        <th class="py-3 px-4 w-[20%]">Harga @gram</th>
                        <th class="py-3 px-4 w-[12%] text-center">Status</th>
                        <th class="py-3 px-4 w-[18%] text-center">Actions</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800 text-sm">

                    <tr v-if="isLoading && paginatedProduk.length === 0">
                    ...
                    <tr v-else v-for="(item, index) in paginatedProduk" :key="item.id"
                        ...
                        <td class="py-3 px-4 font-medium text-slate-900 dark:text-slate-200 uppercase">
                            <div class="flex flex-col items-center justify-center gap-1.5 w-fit mx-auto">
                                ...
                                <div
                                    class="relative w-20 h-8 flex-shrink-0 flex items-center justify-center bg-white dark:bg-slate-800 rounded p-0.5 border border-slate-100 dark:border-slate-700">
                                    <img :src="`/storage/produk/barcode/${item.kodeproduk}.png?t=${new Date().getTime()}`"
                                        alt="Barcode" class="w-full h-full object-contain"
                                        @error="(e) => e.target.src = '/images/default-barcode.png'" />
                                </div>

                                <span
                                    class="font-semibold text-slate-800 dark:text-slate-200 text-[11px] tracking-widest whitespace-nowrap">
                                    {{ item.kodeproduk }}
                                </span>
                        ...
                        <td class="py-3 px-4 font-medium text-slate-900 dark:text-slate-200 uppercase">
                            <div class="flex items-center gap-3">
                                <div class="relative w-9 h-9 flex-shrink-0">
                                    <img v-if="item.image"
                                        :src="`/storage/produk/image/${item.image}?t=${new Date().getTime()}`"
                                        alt="Avatar"
                                    ...
                                    <div v-else
                                        class="w-full h-full rounded-full bg-indigo-100 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-xs font-bold">
                                        {{ item.nama.charAt(0) }}
                                    </div>
                                </div>
                                <div class="flex flex-col min-w-0">
                                    <span class="truncate font-semibold text-slate-800 dark:text-slate-200">{{ item.nama
                                    }}</span>
                                    <span class="text-xs text-slate-400 normal-case tracking-wide font-normal">{{
                                        item.jenisproduk?.jenis }}</span>
                                </div>
                            </div>
                        </td>

                        <td class="py-3 px-4 font-medium text-slate-600 dark:text-slate-400 uppercase truncate">
                            {{ item.berat || '-' }}
                        </td>
                    ...
                    <tr v-if="!isLoading && paginatedProduk.length === 0">
                        <td colspan="8" class="py-10 text-center text-slate-400 dark:text-slate-500 text-xs">
                            No data available.
                        </td>
                    </tr>
                </tbody>
...
import { useProduk } from '../composables/useProduk'; // Disesuaikan ke composable pegawai
