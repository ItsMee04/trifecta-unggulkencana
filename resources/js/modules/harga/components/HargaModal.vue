        ...
        <div class="flex min-h-full items-center justify-center p-4 text-center">
            ...
            <Transition appear enter-active-class="ease-out duration-300"
                ...
                <div v-show="isModalOpen"
                    ...
                    <form @submit.prevent="submitHarga" class="p-6 space-y-4 font-sans">
                        <div class="space-y-1.5 relative">
                            ...
                            <div class="relative">
                                <button type="button" @click="isKaratDropdownOpen = !isKaratDropdownOpen"
                                    ...
                                    <span :class="{ 'text-slate-400': !formHarga.karat_id }">
                                        {{ labelKaratTerpilih }}
                                    </span>
                                    <ChevronDown
                            ...
                            <div v-if="isKaratDropdownOpen"
                                ...
                                <ul
                                    ...
                                    <li v-for="kar in filteredDaftarKarat" :key="kar.id" @click="pilihKarat(kar)"
                                        ...
                                        :class="{ 'bg-indigo-50/50 dark:bg-indigo-950/20 text-indigo-600 dark:text-indigo-400 font-bold': formHarga.karat_id === kar.id }">
                                        <span>{{ kar.karat.toUpperCase() }}</span>
                                        <Check v-if="formHarga.karat_id === kar.id"
                        ...
                        <div class="space-y-1.5 relative">
                            ...
                            <div class="relative">
                                <button type="button" @click="isJenisKaratDropdownOpen = !isJenisKaratDropdownOpen"
                                    ...
                                    <span :class="{ 'text-slate-400': !formHarga.jeniskarat_id }">
                                        {{ !formHarga.karat_id ? '-- Pilih Karat Terlebih Dahulu --' :
                            ...
                            <div v-if="isJenisKaratDropdownOpen"
                                ...
                                <div class="relative shrink-0">
                                    <Search
                                        class="w-3.5 h-3.5 text-slate-400 absolute left-2.5 top-1/2 -translate-y-1/2" />
                                    <input v-model="searchJenisKaratQuery" type="text" placeholder="Cari jenis karat..."
                                        class="w-full pl-8 pr-3 py-1.5 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-lg text-xs focus:outline-none focus:border-indigo-500 text-slate-900 dark:text-white transition" />
                                </div>
                                ...
                                <ul
                                    ...
                                    <li v-for="jkar in filteredDaftarJenisKarat" :key="jkar.id"
                                        ...
                                        class="px-3 py-2 hover:bg-indigo-50 dark:hover:bg-indigo-950/40 text-slate-700 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 rounded-lg cursor-pointer flex items-center justify-between font-medium transition"
                                        :class="{ 'bg-indigo-50/50 dark:bg-indigo-950/20 text-indigo-600 dark:text-indigo-400 font-bold': formHarga.jeniskarat_id === jkar.id }">
                                        <span>{{ jkar.jenis.toUpperCase() }}</span>
                                        <Check v-if="formHarga.jeniskarat_id === jkar.id"
                                            class="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" />
                                    </li>

                                    <li v-if="filteredDaftarJenisKarat.length === 0"
                                        class="px-3 py-4 text-center text-slate-400 dark:text-slate-500 italic">
                            ...
                            <span v-if="errors.jeniskarat_id" class="text-xs text-rose-500 font-medium block mt-1">{{
                                errors.jeniskarat_id }}</span>
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input v-model="formHarga.harga" type="text" id="harga" placeholder="Harga"
                            ...
                            <span v-if="errors.harga" class="text-xs text-rose-500 font-medium block mt-1">
                                {{ errors.harga }}
                            </span>
                        </div>
...
const { isModalOpen, isEdit, formHarga, submitHarga, errors, isLoading, closeModal } = useHarga();
...
const labelKaratTerpilih = computed(() => {
    if (!formHarga.karat_id || daftarKarat.value.length === 0) return '-- Pilih Karat --';
    const ditemukan = daftarKarat.value.find(k => k.id === formHarga.karat_id);
    return ditemukan ? ditemukan.karat.toUpperCase() : '-- Pilih Karat --';
});

const labelJenisKaratTerpilih = computed(() => {
    if (!formHarga.jeniskarat_id || daftarJenisKarat.value.length === 0) return '-- Pilih Jenis Karat --';
    const ditemukan = daftarJenisKarat.value.find(jk => jk.id === formHarga.jeniskarat_id);
...
const filteredDaftarKarat = computed(() => {
    ...
    return daftarKarat.value.filter(k =>
        (k.karat || '').toLowerCase().includes(q)
    );
});

// 🌟 PERBAIKAN UTAMA: Filter jenis karat berdasarkan karat_id terpilih DAN keyword pencarian
const filteredDaftarJenisKarat = computed(() => {
    // 1. Jika karat belum dipilih, jangan tampilkan opsi apa pun
    if (!formHarga.karat_id) return [];

    // 2. Filter berdasarkan karat_id yang sedang dipilih
    let hasil = daftarJenisKarat.value.filter(jk => jk.karat_id === formHarga.karat_id);

    // 3. Saring lagi berdasarkan keyword pencarian jenis karat jika ada
    const q = searchJenisKaratQuery.value.toLowerCase().trim();
    if (q) {
        hasil = hasil.filter(jk => (jk.jenis || '').toLowerCase().includes(q));
    }
...
const pilihKarat = (kar) => {
    // Jika ganti karat, otomatis reset jenis karat yang lama
    if (formHarga.karat_id !== kar.id) {
        formHarga.jeniskarat_id = null;
    }
    formHarga.karat_id = kar.id;
    isKaratDropdownOpen.value = false; // Tutup dropdown
    searchKaratQuery.value = ''; // Reset keyword pencarian
};

const pilihJenisKarat = (jkarat) => {
    formHarga.jeniskarat_id = jkarat.id; // Perbaikan parameter
    isJenisKaratDropdownOpen.value = false; // Tutup dropdown
    searchJenisKaratQuery.value = ''; // Reset keyword pencarian
};
...
onMounted(async () => {
    ...
    try {
        const res = await jeniskaratService.getJenisKarat();
        daftarJenisKarat.value = res.data || res;
    } catch (err) {
        console.error('Gagal memuat list jenis karat:', err);
    }
});
