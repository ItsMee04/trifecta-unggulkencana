        ...
        <div class="flex min-h-full items-center justify-center p-4 text-center">
            ...
            <Transition appear enter-active-class="ease-out duration-300"
                ...
                <div v-show="isModalOpen"
                    class="bg-white dark:bg-slate-900 w-full max-w-xl rounded-2xl border border-slate-100 dark:border-slate-800 shadow-xl relative z-10 overflow-hidden text-left transition-all">
                    ...
                    <div
                        ...
                        <h2 class="font-bold text-slate-900 dark:text-white text-base">
                            {{ isEdit ? 'EDIT PESAN' : 'TAMBAH PESAN' }}
                        </h2>
                    ...
                    <form @submit.prevent="submitPesan" class="p-6 space-y-4 font-sans">

                        <div class="space-y-1.5">
                            <label for="judul" class="text-sm font-medium text-slate-900 dark:text-slate-200 block">
                                Judul Pesan
                            </label>
                            <input v-model="formPesan.judul" type="text" id="judul" placeholder="Judul"
                            ...
                            <span v-if="errors.judul" class="text-xs text-rose-500 font-medium block mt-1">
                                {{ errors.judul }}
                            </span>
                        </div>

                        <div class="space-y-1.5">
                            <label class="text-sm font-medium text-slate-900 dark:text-slate-200 mb-3 block">Pesan</label>
                            <textarea v-model="formPesan.pesan" rows="2" placeholder="Masukkan text pesan"
                            ...
                            <span v-if="errors.pesan" class="text-xs text-rose-500 font-medium block mt-1">{{
                                errors.pesan }}</span>
                        </div>
...
const { isModalOpen, isEdit, formPesan, errors, isLoading, submitPesan, closeModal } = usePesan();
</script>
