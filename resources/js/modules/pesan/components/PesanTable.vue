<template>
    <div
        ...
        <div
            ...
            <h2 class="font-bold text-slate-900 dark:text-white text-base">Daftar Pesan / Template Pesan</h2>
        ...
        <div class="w-full overflow-x-auto relative">
            ...
            <div v-if="isLoading && paginatedPesan.length > 0"
            ...
            <table class="w-full text-left border-collapse min-w-[600px]">
                <thead>
                    <tr
                        ...
                        <th class="py-3 px-4 w-[25%]">Judul</th>
                        <th class="py-3 px-4 w-[45%]">Pesan</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800 text-sm">
                    ...
                    <tr v-if="isLoading && paginatedPesan.length === 0">
        ...
        <div
            ...
            <div class="text-xs text-slate-500">
                Menampilkan
                <span class="font-semibold">{{ showingItemsPesan }}</span>
                dari
                <span class="font-semibold">{{ totalItemsPesan }}</span>
                data
            </div>
            <div class="flex items-center gap-1">
                <!-- First -->
                <button @click="goFirstPesan" :disabled="currentPagePesan === 1"
                ...
                <!-- Prev -->
                <button @click="prevPagePesan" :disabled="currentPagePesan === 1"
                    class="w-8 h-8 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex items-center justify-center disabled:opacity-40">
                    <ChevronLeft class="w-4 h-4" />
                </button>
                <!-- Nomor Halaman -->
                <button v-for="page in visiblePagesPesan" :key="page" @click="currentPagePesan = page" :class="[
                    'w-8 h-8 rounded-lg text-xs font-semibold transition',
                    currentPagePesan === page
                ...
                <!-- Next -->
                <button @click="nextPagePesan" :disabled="currentPagePesan === totalPagesPesan"
                    class="w-8 h-8 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex items-center justify-center disabled:opacity-40">
                    <ChevronRight class="w-4 h-4" />
                </button>
                <!-- Last -->
                <button @click="goLastPesan" :disabled="currentPagePesan === totalPagesPesan"
                    class="w-8 h-8 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex items-center justify-center disabled:opacity-40">
