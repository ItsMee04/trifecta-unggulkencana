<template>
    <div
        ...
        <div
            ...
            <h2 class="font-bold text-slate-900 dark:text-white text-base">Daftar Buku Rekening / Saldo</h2>
        ...
        <div class="w-full overflow-x-auto relative">
            ...
            <div v-if="isLoading && paginatedRekening.length > 0"
            ...
            <table class="w-full text-left border-collapse min-w-[600px]">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800 text-sm">
                    ...
                    <tr v-if="isLoading && paginatedRekening.length === 0">
                    ...
                    <tr v-else v-for="(item, index) in paginatedRekening" :key="item.id"
        ...
        <div
            ...
            <div class="text-xs text-slate-500">
                Menampilkan
                <span class="font-semibold">{{ showingItemsSaldo }}</span>
                dari
                <span class="font-semibold">{{ totalItemsSaldo }}</span>
                data
            </div>
            <div class="flex items-center gap-1">
                <!-- First -->
                <button @click="goFirstSaldo" :disabled="currentPageSaldo === 1"
                ...
                <!-- Prev -->
                <button @click="prevPageSaldo" :disabled="currentPageSaldo === 1"
                    class="w-8 h-8 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex items-center justify-center disabled:opacity-40">
                    <ChevronLeft class="w-4 h-4" />
                </button>
                <!-- Nomor Halaman -->
                <button v-for="page in visiblePagesSaldo" :key="page" @click="currentPageSaldo = page" :class="[
                    'w-8 h-8 rounded-lg text-xs font-semibold transition',
                    currentPageSaldo === page
                ...
                <!-- Next -->
                <button @click="nextPageSaldo" :disabled="currentPageSaldo === totalPagesSaldo"
                    class="w-8 h-8 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex items-center justify-center disabled:opacity-40">
                    <ChevronRight class="w-4 h-4" />
                </button>
                <!-- Last -->
                <button @click="goLastSaldo" :disabled="currentPageSaldo === totalPagesSaldo"
                    class="w-8 h-8 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex items-center justify-center disabled:opacity-40">
