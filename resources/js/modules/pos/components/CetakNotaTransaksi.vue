    ...
    <div class="min-h-screen bg-zinc-800 flex items-center justify-center p-6 print:bg-white print:p-0">
        ...
        <div v-if="isLoading" class="text-white font-semibold animate-pulse print:hidden">
            <div class="flex flex-col items-center gap-3">
                ...
                <span>Menyiapkan Preview Nota...</span>
            </div>
        </div>
                    ...
                    <table class="text-[9px] text-left">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="py-0.5 font-bold uppercase text-slate-900">: {{ notaData.nama_pelanggan }}
                                </td>
                            </tr>
                            <tr>
                                <td class="py-0.5 pr-2 text-slate-500">Alamat / Kota</td>
                                <td class="py-0.5 text-slate-700 uppercase font-semibold">: {{ notaData.alamat_pelanggan
                                    }}</td>
                            </tr>
                            <tr>
                                <td class="py-0.5 pr-2 text-slate-500">No. HP</td>
                                <td class="py-0.5 text-slate-700 font-mono font-semibold">: {{ notaData.hp_pelanggan }}
                                </td>
                            </tr>
                            <tr>
                                <td class="py-0.5 pr-2 text-slate-500">No. Transaksi</td>
                                <td class="py-0.5 font-mono font-bold text-slate-800">: {{ notaData.kode_transaksi }}
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    ...
                    <thead class="w-full">
                        <tr
                            ...
                            <th class="border-r border-slate-300 py-1.5 w-[15%] flex items-center justify-center">Foto
                                Produk</th>
                            <th class="border-r border-slate-300 py-1.5 w-[40%] text-left px-3 flex items-center">Uraian
                                Produk</th>
                            <th class="border-r border-slate-300 py-1.5 w-[10%] flex items-center justify-center">Berat
                            </th>
                            <th class="border-r border-slate-300 py-1.5 w-[10%] flex items-center justify-center">Karat
                            </th>
                            <th
                                class="border-r border-slate-300 py-1.5 w-[12%] text-right pr-3 flex items-center justify-end">
                                Harga @Gram</th>
                            <th class="border-r border-slate-300 py-1.5 w-[10%] flex items-center justify-center">Diskon
                            </th>
                            <th class="py-1.5 w-[13%] text-right pr-3 flex items-center justify-end">Total</th>
                        </tr>
                    </thead>
            ...
            <div class="grid grid-cols-12 gap-3 items-end border-t border-slate-200 pt-2">
                <div class="col-span-5 text-[7px] text-slate-500 space-y-0.5 self-start">
                    ...
                    <div class="mt-2 text-[8px] font-mono text-slate-400 bg-slate-50 px-1 py-0.5 rounded inline-block">
                        ID: <span class="font-bold">{{ generateHash(notaData.kode_transaksi) }}</span>
                    </div>
                </div>
...
// 1. Deklarasi file logo (pastikan ditaruh di folder public/assets/report/ atau sesuaikan path-nya)
const logoToko = ref(logoTokoImg);

// 2. Helper memformat Tanggal (misal: "17 Juli 2026")
...
// 3. Helper memformat Rupiah (misal: "Rp 1.500.000")
const formatRupiah = (angka) => {
    if (angka === undefined || angka === null || isNaN(angka)) return 'Rp 0';
    return 'Rp ' + Number(angka).toLocaleString('id-ID');
};

// 4. Helper generate Hash sederhana untuk ID validasi nota unik
...
const fetchNotaRealData = async () => {
    ...
    try {
        ...
        if (response && response.status) {
            // Karena stored procedure Anda langsung me-return 1 baris record data (bukan array di dalam array),
            // kita langsung petakan ke notaData.
            notaData.value = response.notaData;

            // Set judul tab browser agar dinamis
            document.title = `Nota_${notaData.value.kode_transaksi}`;
        }
    } catch (error) {
        console.error("Gagal memuat data nota dari server:", error);
    } finally {
        isLoading.value = false;

        // Auto-print setelah data ter-load sempurna
        if (notaData.value) {
            setTimeout(() => {
                window.print();
            }, 500);
