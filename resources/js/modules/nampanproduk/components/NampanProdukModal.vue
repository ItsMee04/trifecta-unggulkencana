                ...
                <div v-show="isModalOpen"
                    ...
                    <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-[11px] text-slate-400 mt-0.5">Pilih satu atau beberapa produk di bawah ini
                            </p>
                        </div>
                    ...
                    <form @submit.prevent="handleSubmit" class="p-6 space-y-4 font-sans">
                        ...
                        <div class="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
                            ...
                            <div class="text-xs text-slate-400 self-center">
                                Terpilih: <span class="font-bold text-indigo-600 dark:text-indigo-400">{{
                                    selectedProdukIds.length }}</span> produk
                            </div>
                        </div>

                        <div v-if="isLoadingProduk" class="space-y-2 py-8 text-center text-sm text-slate-400">
                            <div
                                class="inline-block animate-spin rounded-full h-5 w-5 border-2 border-indigo-500 border-t-transparent mb-2">
                            </div>
                            <div>Memuat data produk master...</div>
                        </div>

                        <div v-else-if="paginatedProduk.length === 0"
                            class="py-12 text-center text-slate-400 dark:text-slate-500 text-sm italic">
                        ...
                        <div v-else class="overflow-x-auto border border-slate-100 dark:border-slate-800 rounded-xl">
                            <table class="w-full text-left border-collapse text-sm text-slate-600 dark:text-slate-400">
                                <thead
                                    class="bg-slate-50/70 dark:bg-slate-800/40 text-slate-700 dark:text-slate-300 font-semibold text-xs border-b border-slate-100 dark:border-slate-800">
                                    <tr>
                                ...
                                <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60">
                                    <tr v-for="item in paginatedProduk" :key="item.id"
                                        ...
                                        <td class="p-3 font-mono text-xs text-slate-900 dark:text-white font-semibold">
                                            {{ item.kodeproduk }}</td>
                        ...
                        <div v-if="filteredProduk.length > 0"
                            ...
                            <div class="text-xs text-slate-500">
                                Menampilkan
                                <span class="font-semibold">{{ showingItemsProduk }}</span>
                                dari
                                <span class="font-semibold">{{ totalItemsProduk }}</span>
                                data
                            </div>
...
<script setup>
import { X, Search, ChevronLeft, ChevronRight, ChevronsRight, ChevronsLeft } from 'lucide-vue-next';
